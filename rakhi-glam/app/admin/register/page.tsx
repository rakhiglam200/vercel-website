...
export default function AdminRegisterPage() {
  ...
  const [email, setEmail] = useState("rakhiglam200@gmail.com");
    ...
    <div className="min-h-screen bg-[var(--color-beige)] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-2">Create Admin Account</p>
        </div>

        <div className="bg-white rounded-2xl p-8 border border-[var(--color-border)] shadow-sm">
          <h1 className="font-heading text-xl font-bold text-[var(--color-navy)] mb-6">Register</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--color-navy)] mb-1">Admin Email</label>
              <input
                type="email"
                value={email}
          ...
          <p className="text-xs text-[var(--color-text-muted)] text-center mt-4">
            <Link href="/admin/login" className="text-[var(--color-navy)] hover:underline">Already have an account? Sign in</Link>
          </p>
        </div>
      </div>
    </div>
